    ...
    for event in eventDict:
        ...
        if currentUser in openSessions:
            ...
            if(idleTime > session_idle_threshold):
                ...
                #######
                openSessions[currentUser]['numOfEvents']=len(openSessions[currentUser]['events'])

                #add to the closed sessions(finished)
                closedSessions.append(openSessions[currentUser]) ## maybe it will be better to write 'closed sessions' to file here
            ...
            #If the event is part of the session append it in the user session events
                ...
                openSessions[currentUser]['isBuySession']=(openSessions[currentUser]['isBuySession'] or isBuyEvent) ## might be upgraded to keep total amount of sales or number of sales
                ...
                # update num of dwells,average dwell,min dwell, max dwell
...
def addNewSession(event, currentUser,openSessions,isBuyEvent):
    ...
    openSessions[currentUser]['numOfEvents']=0 #update the number when session is saved
...
def updateGlobalDwell(accumulatedData,currentDwell):
    ...
    if(accumulatedData['numOfDwells']>1):
        accumulatedData['averageDwell']=averageDwell*(numOfDwells-1)/numOfDwells+1/numOfDwells*currentDwell
    else:
...
def updateSessionDwell(openSessions,currentDwell,currentUser):
    ...
    if('totalDwell' not in openSessions[currentUser]):
